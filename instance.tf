...
resource "vsphere_virtual_machine" "vm_deploy" {
  name             = "${local.vm_prefix}-${random_string.folder_name_prefix.id}-dbvm"
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = "${local.vm_prefix}-${random_string.folder_name_prefix.id}-dbvm"
...
resource "null_resource" "vm_node_init" {
  ...
  provisioner "remote-exec" {
    inline = [
        ...
        "/tmp/installmysql.sh ${local.mysql_pass} teadb teauser teapassword ${vsphere_virtual_machine.vm_deploy.default_ip_address}",
    ]
    connection {
      type = "ssh"
      host = "${vsphere_virtual_machine.vm_deploy.default_ip_address}"
